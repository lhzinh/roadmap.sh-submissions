...
result = llm.call(
    messages=[
        {"role": "user", "content": f"Extract fields from this email: {email_text}"}
    ],
...
class ResearchCrew:
    ...
    def reporting_task(self) -> Task:
        return Task(
            ...
            context=[
                self.research_task()
            ],  # Explicit dependency (optional in sequential)
            output_file="output/report.md",
        )
...
class AnalysisFlow(Flow[AnalysisState]):
    ...
    def research(self):
        researcher = Agent(
            role="Researcher", goal="...", backstory="...", tools=[SerperDevTool()]
        )
        result = researcher.kickoff(
...
class ProductFlow(Flow[ProductState]):
    ...
    def handle_complex(self):
        ...
        result = ResearchCrew().crew().kickoff(inputs={"topic": self.state.request})
        self.state.answer = result.raw
```
...
class QualityFlow(Flow[QAState]):
    ...
    def revise(self):
        result = EditorCrew().crew().kickoff(inputs={"draft": self.state.draft})
        self.state.draft = result.raw
```
